...
import { create_internships_question_api, edit_internships_question_api, internships_question_api } from "../api/internship";
import { notificationHandler } from "../../utils/Notification";
const IntrenQuestionList = () => {
  ...
  async function fetchAllquestinLIst(data) {
    setisLoading(true);
    try {
      const temp = {
        search: data ? data : "",
      ...
      let res = await internships_question_api(temp);
      if (res.data.status) {
  ...
  const create_community_questionn_func = async () => {
    ...
    try {
      ...
      let res = await create_internships_question_api(temp);
      if (res.data.status) {
        console.log(res);
        fetchAllquestinLIst();
        setquestion("");
  ...
  const intrenship_status_update = async (data) => {
    setisLoading(true);
    let temp = {
      id: data.id,
    ...
    try {
      let res = await edit_internships_question_api(temp);
      if (res.data.status) {
        setisLoading(false);
        fetchAllquestinLIst();
  ...
  return (
    <div className="container">
      <div className={s["article-list-title"]}>
        <h3>Internship Question List</h3>
      </div>
      <div className={s["user-list-heading"]}>
        <div className={s["user-list-title"]}>
          <div className="beat_left">
            <div
              ...
              onClick={() => {
                settype("Add");
                setquestionexpandbox(!questionexpandbox);
              }}
            >
              <IoMdAdd /> Add
            </div>
        ...
        <div className={s["user-list-search"]}>
          <div className={s["search-box"]}>
            ...
            <input type="text" spellCheck="false" onChange={(e) => fetchAllquestinLIst(e.target.value)} placeholder="Search by name..." />
          </div>
        </div>
      </div>
              ...
              <StyledTableRow key={row.id}>
                ...
                <StyledTableCell align="center">
                  ...
                  <div
                    ...
                    onClick={() => intrenship_status_update(row)}
...
export default IntrenQuestionList;
